...
// we consider the worst case scenario of the merging step as being n.
// we consider the worst case scenario of a divide and conquer step as being log(n)
// since both are dependent on n and the merging step is not a coefficient, we can multiply our divide and conquer complexity.

// space complexity: 0(n) ?
// worst case scenario, we are storing the full length of n in memory multiplied by a constant stack length? Since this is a constant, we drop the multiplication.
...
function mergeHelper(arr1, arr2) {
  ...
  mergedArray.push(...arr1.slice(p1));
  mergedArray.push(...arr2.slice(p2));
  return mergedArray;
}
...
console.log(mergeSort(["abb", "abc", "bac", "aab", "aaa", "a", 'b'])); //[ 'a', 'aaa', 'aab', 'abb', 'abc', 'bac' ]
console.log(mergeSort(["peach", "straw", "apple", "spork", "sss"])); // [apple, aardvark, sammy, spork]


//create 1m random numbers
var randomArr = [...Array(1000000)].map(() => Math.floor(Math.random() * 1000000));
const res = mergeSort(randomArr)
