...
LRUCache.prototype.size = function () {
  return(Object.keys(this.nodeReferences).length)
};

LRUCache.prototype.get = function (key) {
  if (this.nodeReferences[key]) {
    [val, node] = this.nodeReferences[key]
    this.priorityList.moveToEnd(node)
    return val;
  } else {
    return null;
  }
...
LRUCache.prototype.set = function (key, val) {
  if (this.size() < this.limit) {
    ...
    this.priorityList.push(key)
    // add the key and value to the hashmap
    this.nodeReferences[key] = [val, this.priorityList.tail];
  } else {
    // if full, get the key associated with oldest item:
    const keyToDelete = this.priorityList.head.val;
    console.log('removing key: ', keyToDelete) 
    // shift from the head to remove oldest
    this.priorityList.shift();
    // the key from the hashmap
    delete this.nodeReferences[keyToDelete];
...
// which takes priority.
cache.set("item5", 5);
console.log('set item5, remove item4', cache.nodeReferences['item4'])
 // item3 is also removed, because it was retrieved before item2 was
// last retrieved.
cache.set("item6", 6);
console.log('set item6, remove item 2', cache.nodeReferences['item3'])